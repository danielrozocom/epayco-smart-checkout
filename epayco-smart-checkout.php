...
function epayco_scs_check_github_update($transient)
{
  ...
  $api_url     = 'https://api.github.com/repos/' . EPAYCO_SCS_GITHUB_REPO . '/releases/latest';
  ...
  if (!version_compare($latest_version, EPAYCO_SCS_VERSION, '>'))
    return $transient;

  // Look for an attached ZIP asset (generated by GitHub Actions)
  $zip_url = null;
  ...
  // Fallback to GitHub's auto-generated zipball (may need folder rename)
  if (!$zip_url)
...
function epayco_scs_plugin_info($res, $action, $args)
{
  ...
  $release = json_decode(wp_remote_retrieve_body($response), true);
  $version = ltrim($release['tag_name'] ?? EPAYCO_SCS_VERSION, 'v');
  ...
  return (object) [
    ...
    'requires_php'  => '7.4',
    'sections'      => [
      'description' => $release['body'] ?? 'Plugin para integrar ePayco Smart Checkout v2 en WordPress.',
