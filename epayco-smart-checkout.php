...
define('EPAYCO_SCS_VERSION', '1.2.9');
define('EPAYCO_SCS_GITHUB_REPO', 'danielrozocom/epayco-smart-checkout');

/**
 * GitHub Auto-Updater
 * Hooks into WordPress's update mechanism to detect newer releases on GitHub.
 * When a new tag (e.g. v1.2.4) is published in the repo, WordPress will show
 * the "Update available" notice and allow one-click updating from the Plugins page.
 */
add_filter('pre_set_site_transient_update_plugins', 'epayco_scs_check_github_update');
function epayco_scs_check_github_update($transient)
{
  if (empty($transient->checked))
    return $transient;

  $plugin_slug = plugin_basename(__FILE__);
  $api_url     = 'https://api.github.com/repos/' . EPAYCO_SCS_GITHUB_REPO . '/tags';
  ...
  foreach ($tags as $tag) {
    ...
    if (version_compare($ver, $latest_version, '>')) {
      $latest_version = $ver;
      $latest_tag = $tag;
    }
  }

  if (!$latest_tag || !version_compare($latest_version, EPAYCO_SCS_VERSION, '>'))
    return $transient;

  $transient->response[$plugin_slug] = (object) [
    'slug'        => 'epayco-smart-checkout',
    'plugin'      => $plugin_slug,
    'new_version' => $latest_version,
    'url'         => 'https://github.com/' . EPAYCO_SCS_GITHUB_REPO,
 ...
 * GitHub's zipball extracts to a folder like "danielrozocom-epayco-smart-checkout-abc1234".
 * This filter renames it to the correct "epayco-smart-checkout" so WordPress doesn't lose the plugin path.
 */
add_filter('upgrader_source_selection', 'epayco_scs_fix_upgrade_folder', 10, 4);
function epayco_scs_fix_upgrade_folder($source, $remote_source, $upgrader, $hook_extra)
{
  if (!isset($hook_extra['plugin']) || strpos($hook_extra['plugin'], 'epayco-smart-checkout') === false)
    return $source;

  $correct_folder = trailingslashit(dirname($source)) . 'epayco-smart-checkout/';

  if (basename(untrailingslashit($source)) !== 'epayco-smart-checkout') {
...
/**
 * Show plugin info popup in WordPress (Detalles de la versión link).
 */
add_filter('plugins_api', 'epayco_scs_plugin_info', 20, 3);
function epayco_scs_plugin_info($res, $action, $args)
{
  if ($action !== 'plugin_information' || !isset($args->slug) || $args->slug !== 'epayco-smart-checkout')
    return $res;
  ...
  $version = $latest_tag ? $latest_version : EPAYCO_SCS_VERSION;
  $zip_url = $latest_tag ? $latest_tag['zipball_url'] : '';

  return (object) [
    'name'          => 'ePayco Smart Checkout',
    'slug'          => 'epayco-smart-checkout',
    'version'       => $version,
    'author'        => '<a href="https://danielrozo.com/">Daniel Rozo</a>',
    'homepage'      => 'https://github.com/' . EPAYCO_SCS_GITHUB_REPO,
    'requires'      => '5.8',
    'tested'        => '6.8',
    'requires_php'  => '7.4',
    'sections'      => [
      'description' => 'Plugin para integrar ePayco Smart Checkout v2 en WordPress.',
      'changelog'   => 'Ver historial de cambios en GitHub: https://github.com/' . EPAYCO_SCS_GITHUB_REPO . '/blob/main/readme.txt',
    ],
